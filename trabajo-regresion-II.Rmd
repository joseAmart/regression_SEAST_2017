...
```{r}
## Completar aqui
GradosTeleco <- read.table(file = "data/notamediagradosETSIT-completo.csv", header=TRUE, sep=";",    stringsAsFactors = FALSE)
```
...
errores.predict.PAU<-abs(test.predict.PAU - GradosTeleco.test$notamedia) 
```
...
errores %>% ggplot(aes(x=errores)) + geom_density(fill = "blue", alpha = 0.5)
```
...
resumenerrores <- errores %>% 
  ...
  summarise (error_cuadratico_medio = mean(errores^2, na.rm = TRUE),
             por25 = sum(abs(errores) < 0.25, na.rm = TRUE) / length(errores) *100,
             por10 = sum(abs(errores) < 0.10, na.rm = TRUE) / length(errores) *100)
```
...
training.lm.PAU.primercurso <- lm(data = GradosTeleco.training, notamedia ~ PAU_CALIFICACION_NOTA_MATERIA + PAU_CALIFICACION_NOTA_MATERIA_DECIL + FISICA_NOTA_MATERIA + FISICA_NOTA_MATERIA_DECIL + MATEMATICAS_II_NOTA_MATERIA + MATEMATICAS_II_NOTA_MATERIA_DECIL+ SISTEMAS_Y_CIRCUITOS_NOTA+ FUNDAMENTOS_DE_PROGRAMACION_NOTA)
...
errores.predict.PAU.primercurso<-abs(test.predict.PAU.primeroscursos - GradosTeleco.test$notamedia) 
```
...
errores.predict.completo<-abs(test.predict.completo - GradosTeleco.test$notamedia)
```
...
### Completar aquí
errores <- rbind(errores,
                 data.frame(caracteristicas = "Todas",
...
### Completar aquí, usando ggplot con geom_density,
errores %>% 
  ggplot(aes(x = errores, fill = caracteristicas)) +geom_density(alpha = 0.5)
```
